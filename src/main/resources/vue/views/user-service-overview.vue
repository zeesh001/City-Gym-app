<template id="user-service-overview">
  <app-layout>
...
<script>
Vue.component("user-service-overview", {
  template: "#user-service-overview",
  data: () => ({
    service: null,
    noUserFound: false,
  }),
  created: function () {
    const userId = this.$javalin.pathParams["service_name"];
    const url = `/api/services/${userId}`
    axios.get(url)
